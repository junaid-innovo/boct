...
class Map extends PureComponent {
   ...
   getCalcultedTimeOfWaypoints = (routelist, store_address) => {
      ...
      for (let i = 0; i < allorders.length; i++) {
         ...
         if (typeof allorders[i + 1] !== 'undefined') {
            ...
            count++
            let orderpoints = this.createOrderObject(
               originorder,
               destinationorder
            )
   ...
   componentDidUpdate(prevProps, prevState) {
      ...
      if (this.props.routelist) {
         ...
         if (
            ...
            if (
               this.props.routelist.deliveries !==
                  this.state.routelist.deliveries ||
               this.props.routelist.deliveries !==
                  prevProps.routelist.deliveries
            ) {
               this.setState({
         ...
         >
            ...
            >
               ...
               <div>
                  ...
                  <span>
                     {typeof this.state.selectedVehilce.tripCounter !==
                        'undefined' &&
               ...
               <div>
                  ...
                  <span>
                     {typeof this.state.selectedVehilce.tripCounter !==
                        'undefined' &&
                        this.state.selectedVehilce.tripCounter
                           .completed_counter}
                  </span>
               </div>{' '}
               <div>
         ...
         if (position === 'top_right') {
            map.controls[this.google.maps.ControlPosition.TOP_RIGHT]['i'].map(
               (val, i) => {
                  ...
                  if (val.accessKey === accessKey) {
                     map.controls[
                        this.google.maps.ControlPosition.TOP_RIGHT
                     ].removeAt(i)
                  }
               }
            )
   ...
   polygonClick = (e, overlay) => {
         ...
         <React.Fragment>
            ...
            <div className="btn-group" role="group" aria-label="First group">
               ...
               <button
                  ...
                  onClick={(e) =>
                     this.clearGeoFenceRoute(e, overlay, accessKey)
                  }
               >
                  Clear Route
               </button>
      ...
      let index = map.controls[
         this.google.maps.ControlPosition.TOP_CENTER
      ].push(controlButtonDiv)
      ...
      return (
         <React.Fragment>
            ...
            >
               ...
               {this.state.mapfeatures.showRoutes &&
                  this.showDirectionRendrer()}
               {this.state.vehicleTracking.length > 0 &&
                  this.renderVehicleTrack()}
               {this.renderVehicleInfoWindow()}
               {this.state.mapfeatures.showOriginMarker &&
