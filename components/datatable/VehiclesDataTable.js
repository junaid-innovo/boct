...
class BootstrapDataTable extends Component {
  ...
  renderDataTable = () => {
    ...
    if (this.props.data) {
      let products = [];
      products = _.uniqBy(this.props.data, "vehicle_id");
      //   if (this.props.dataFor === "orders") {
      ...
      const selectRow = {
        ...
        onSelect: (row, isSelect, rowIndex, e) => {
          if (isSelect) {
            ...
            let selectedVehcileIds = [...this.state.selectedVehcileIds];
            if (!selectedVehcileIds.includes(vehicle_id)) {
              selectedVehcileIds.push(vehicle_id);
            }
            if (this.props.sendSelectedVehcileIds) {
              this.props.sendSelectedVehcileIds(selectedVehcileIds);
            }
            this.setState({
              selectedVehcileIds: selectedVehcileIds,
            });
          } else {
            let vehicle_id = row.vehicle_id;
            ...
            let selectedVehcileIds = [...this.state.selectedVehcileIds];
            if (selectedVehcileIds.includes(vehicle_id)) {
              _.remove(selectedVehcileIds, (item) => item == vehicle_id);
            }
            this.props.sendSelectedVehcileIds(selectedVehcileIds);
            this.setState({
              selectedVehcileIds: selectedVehcileIds,
        ...
        onSelectAll: (isSelect, rows, e) => {
          if (isSelect) {
            let selectedVehciles = this.state.selectedVehcileIds;
            let selectedVehcilesIds = _.map(products, "vehicle_id");
            let finalVehcilesIds = [
              ...this.state.selectedVehcileIds,
              ...selectedVehcilesIds,
            ];

            this.setState({
              selectedVehcileIds: _.uniq(finalVehcilesIds),
            });
            this.props.sendSelectedVehcileIds(finalVehcilesIds);
          } else {
            let selectedVehciles = this.state.selectedVehcileIds;

            let roworder_ids = _.map(products, "vehicle_id");
            let finalVehcilesIds = _.pullAll(selectedVehciles, roworder_ids);
        ...
        selected: this.state.selectedVehcileIds,
      };
      const defaultSorted = [
        {
      ...
      columns = columns.map((val) => {
        ...
        if (
          val.dataField === "vehicle_category_name" ||
          val.dataField === "vehicle_type_name" ||
          val.dataField === "driver_name"
        ) {
          let updatedval = { ...val };
          updatedval.dataField = `${updatedval.dataField}[${language}]`;
          let text = "";
          if (this.props.t) {
